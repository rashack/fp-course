...
map ::
  ...
  -> List a
  -> List b
map f = foldRight (\ha t -> (f ha :. t)) Nil
...
filter ::
  ...
  -> List a
  -> List a
filter pred = foldRight f Nil
  where
    f e acc = case pred e of
                True  -> (e :. acc)
                False -> acc
...
flatten ::
  List (List a)
  -> List a
flatten = foldRight (\acc l -> acc ++ l) Nil
...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional l = case any nonFull l of
                  True  -> Empty
                  False -> Full (deOptionalize l)
  where
    nonFull Empty = True
    nonFull _     = False
    deOptionalize (Full a :. t) = a :. deOptionalize t
    deOptionalize Nil = Nil
    deOptionalize (Empty :. _) = Nil
